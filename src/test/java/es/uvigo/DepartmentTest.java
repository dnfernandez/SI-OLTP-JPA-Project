...
public class DepartmentTest extends SQLBasedTest {
	...
	public void testCreateDepartment() throws SQLException {
		...
		ResultSet rs = statement.executeQuery("SELECT COUNT(*) as total FROM Department WHERE id = " + d.getId());
		rs.next();

		assertEquals(1, rs.getInt("total"));

		statement = jdbcConnection.createStatement();
		rs = statement.executeQuery("SELECT department_id FROM Employee e WHERE id = " + id);
		rs.next();
	...
	public void testFindDepartment() throws SQLException {
		Statement statement = jdbcConnection.createStatement();
		statement.executeUpdate("INSERT INTO Department(name) values('Projects')", Statement.RETURN_GENERATED_KEYS);
		int deptId = getLastInsertedId(statement);

		statement = jdbcConnection.createStatement();
		statement.executeUpdate("INSERT INTO Employee(name, department_id) values('Daniel', " + deptId + ")",
				Statement.RETURN_GENERATED_KEYS);
	...
	public void testLazyInitializationException() throws SQLException {
		Statement statement = jdbcConnection.createStatement();
		statement.executeUpdate("INSERT INTO Department(name) values('Budget')", Statement.RETURN_GENERATED_KEYS);
		int deptId = getLastInsertedId(statement);
